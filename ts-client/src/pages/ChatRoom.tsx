...
interface ChatRoomProps extends Pick<ChatPageProps, 'socket' | 'username' | 'room' | 'messageList' | 'setMessageList' | 'onlineList' | 'setOnlineList' > {}

function ChatRoom({socket,username,room,messageList,setMessageList,onlineList,setOnlineList}:ChatRoomProps) {
  ...
  useEffect(() => {
    ...
    socket.on("someone-joined", (username, time) => {
        setMessageList(( list: generalMessageType[] ):generalMessageType[] => 
          [...list, { username: username, time: time, join: true }])
    ...
    socket.on("someone-left", (username, time) => {
        setMessageList(( list: generalMessageType[] ):generalMessageType[] => 
          [...list, { username: username, time: time, join: false }])
        setOnlineList((prev:string[]):string[] =>  prev.filter((each) => each != username))
    })

    socket.on("online-member-update", (onlineMemberUpdate:{room:string , members:string[]}[]) => {
  ...
  async function sendMessage() {
    if (file) {
        ...
        setMessageList(( list: generalMessageType[] ):generalMessageType[] =>  [...list, messageData])
        setMessage("")
        setFile(null)
    }
    ...
    <div className='flex flex-row justify-evenly '>
        ...
        <div className='h-[37em] w-[40em] bg-white rounded-3xl flex flex-col items-center relative '>
          ...
          <div className='bg-cyan-50 flex rounded-xl py-2 px-2  items-center mt-auto mb-4  bottom-8 '>
            ...
            <input className='w-96 bg-cyan-50' name="message" onChange={(e) => { setMessage(e.target.value) }} value={message} placeholder="Type something..." onKeyDown={(e:React.KeyboardEvent<HTMLInputElement>) => { keypress(e) }}/>
